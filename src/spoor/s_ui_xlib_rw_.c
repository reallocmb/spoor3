...
struct XlibHandle {
    ...
    .ui_area_head = &(UIArea){
        ...
        .prev = NULL,
        .next = NULL,
        .drawing_func = ui_area_blanc_draw_func,
...
void ui_area_close(void)
{
    ...
    if (current->parent->childs_count == 2)
    {
        ...
        if (current->parent == XlibHandleGlobal.ui_area_head)
        {
            ...
            if (ptr->childs)
            {
                UIArea *childs = ptr->childs;
                while (childs)
                {
                    childs->parent = XlibHandleGlobal.ui_area_head;
                    childs = childs->next;
                }
            }
...
void ui_area_blanc_draw_func(UIArea *ui_area)
{
    ...
    char id_parent_str[20] = { 0 };
    char id_str[20] = { 0 };
    char id_next_str[20] = { 0 };
    char id_prev_str[20] = { 0 };
    sprintf(id_str, "id: %d", ui_area->id);
    if (ui_area->parent)
        sprintf(id_parent_str, "id parent: %d", ui_area->parent->id);
    if (ui_area->next)
        sprintf(id_next_str, "id next: %d", ui_area->next->id);
    if (ui_area->prev)
        sprintf(id_prev_str, "id prev: %d", ui_area->prev->id);
    xlib_text_draw(id_parent_str, ui_area->x + 10, ui_area->y + 10, 0xffffff);
    xlib_text_draw(id_str, ui_area->x + 10, ui_area->y + 10 + 16, 0xffffff);
    xlib_text_draw(id_prev_str, ui_area->x + 10, ui_area->y + 10 + 2 * 16, 0xffffff);
...
void default_input(void)
{
    ...
    switch (XlibHandleGlobal.buffer_command[XlibHandleGlobal.buffer_command_size - 1])
    {
        ...
        {
            printf("a test\n");
            ui_area_append(ui_area_blanc_draw_func, UI_AREA_FLAG_HORIZONTAL);
        } break;
        case 'i':
        {
            printf("i test\n");
            ui_area_append(ui_area_blanc_draw_func, UI_AREA_FLAG_VERTICAL);
        } break;
        case 's':
        {
